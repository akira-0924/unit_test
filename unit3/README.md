...
## 確認(Assert)フェーズの読みやすさの改善

FluentAssertions を利用することで、確認フェーズのコードを読みやすくすることができる。

[test code](./sample3.12.java)

## まとめ

- 単体テストのすべてのテストケースは AAA パターン（準備、実行、確認）を適応して記述するべきである。同じフェーズが複数回ある場合は、複数の 1 単位の振る舞いを検証しようとしていることになるため、修正が必要
- 実行フェースのコードが 1 行を超える場合は注意が必要
- テストを読みやすくするためにテスト対象システムに「sut」(System Under Test)という名前をつけると良い
- テスト・フィクスチャに関するコードを共有するためにテストクラスのファクトリメソッドを導入すると良い（コンストラクタだとテストケース間の結びつきが強くなる）
- 厳格な命名規則に縛られないようにする
- パラメータ化テストを用いることで、テストケースをより少ないコード量で表現できる
