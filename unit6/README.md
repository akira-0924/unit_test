...
[sample code](./sample6.12.java)

今回の修正では`AuditManager`クラスに訪問者記録ファイルの情報を収集させるのではなく、`FileContent`クラスの配列を渡すようにしている。`FileContent`クラスは次に示すように**ファイルシステムに対して何をするのかを決定するのに必要な全ての情報を持っている**。<br>
さらに、`AuditManager`クラスは訪問者記録ファイルに対して何かをするのではなく「何をするのか」（そんな副作用を起こすのか）を支持する決定内容(`FileUpdate`クラス)を返すようにしている。<br>
そしてその決定内容を受け、その決定に基づいた処理を行う（副作用を起こす）のが、`Persister`クラスである。

<br>

注目すべき部分は、この`Persister`クラスが**いかに単純であるか**という部分。if 文がなくビジネスロジックに関する全ての複雑さは`AuditManager`クラスに担っている。これが、**ビジネスロジックと発生する副作用の分離**を実現している。

分離ができたら、関数核と可変殻の連携を指揮するアプリケーション・サービスが必要となる。

[sample code](./sample6.13.java)

このアプリケーションサービスが行っていることは、関数的核と可変殻の連携を指揮している。それに加えて、外部クライアントがこのシステムにアクセスするための入り口をとなる部分も提供する。
このような実装になったことで、単体テストは、「事前条件となる訪問者記録ファイルとしのディレクトリのデータを準備することと、AuditManager クラスで下された決定」を検証することだけとなる。

[sample code](./sample6.14.java)

モックも不要となり、入力値と出力値のみを検証することでテストケースが簡潔になった。
